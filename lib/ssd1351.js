...
async function initializeSpiConnection(maxSpeedHz = 19660800) {
    return new Promise((resolve, reject) => {
        oled = spi.open(0, 0, {
            maxSpeedHz: Math.round(maxSpeedHz / 65536) * 65536 //Divisor is 65536 
        }, async function (err) {
            if (err) {
                console.error(err);
                reject(err);
...
module.exports = class Ssd1351 {
    ...
    /**
     * Constructor
    * @param {*} maxSpeedHz - device clock frequency in Hertz, default system specific
    */
    constructor(rstGpioNb = 25, dcGpioNb = 24, maxSpeedHz = undefined) {
        rstGpioId = rstGpioNb;
        dcGpioId = dcGpioNb;
    ...
    async turnOnDisplay(initialContrast = 0xFF) {
        try {
            ...
            if (undefined === oled) {
                await initializeSpiConnection(maxSpeedHz);
            }
    ...
    async clearDisplay() {
        bytesData = Array(WIDTH * HEIGHT * 2).fill(0);
        cursor_x = 0;
        cursor_y = 0;
    }
     ...
     */
    writeString(font = defaultFont, size, string, colour, wrap, padding, backgroundColour) {
